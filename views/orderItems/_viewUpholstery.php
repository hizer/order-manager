...
if( $data->isNewUpdateDate(CHtml::encode($data->upholstery_updated), $type)){
}else{
	if(Yii::app()->params[$type.'Updated'] != "0000-00-00 00:00:00"){
		
		$updated = CHtml::encode(Yii::app()->dateFormatter->format("y-MM-dd", $data->upholstery_updated));
		echo "<tr><td colspan='4' ><strong>   ";
		echo CHtml::encode(Yii::app()->dateFormatter->format("dd-MM-y", $data->upholstery_updated));
		echo "<td><strong> ";
...
<td>
	<b><?php //echo CHtml::encode($data->getAttributeLabel('upholstery_updated')); ?></b>
	<?php echo CHtml::encode(Yii::app()->dateFormatter->format("HH:mm:ss", $data->upholstery_updated)); ?> 
